...
with Connection.open_serial_port("/dev/ttyUSB0") as connection:
    ...
    axis.settings.set(
        "maxspeed", 20, Units.VELOCITY_MILLIMETRES_PER_SECOND
    ...
    axis.move_relative(
        4, Units.LENGTH_MILLIMETRES
    )  ### NOTE THAT I ANTICIPATE HOME IS THE CORRECT STARTING POS, so this is commented
...
camera_letter_to_number = {"A": 0, "B": 1, "C": 2, "D": 3}
...
def start_camera(camera_letter):
    if camera_letter == "A":
        ...
        print(
            "Letter should be A, B, C, or D. Unrecognized camera letter provided: ",
            camera_letter,
        )
...
def take_still(camera_letter, output_directory):
    timestamped_image = "{camera_letter}-image-{timestamp}.jpg".format(
        camera_letter=camera_letter, timestamp=time.strftime("%Y%m%d-%H%M%S")
    )
    output_file_path = (
        "{output_directory}/camera{camera_letter}/{timestamped_image}".format(
            output_directory=output_directory,
            camera_letter=camera_letter,
            timestamped_image=timestamped_image,
        )
    )
    capture_image_command = "sudo libcamera-still -t 10000 --camera {camera_num} -o {output_file_path}".format(
        camera_num=camera_letter_to_number[camera_letter],
...
def main():
    ...
    for camera_letter in CAMERA_LETTERS:
        ...
        for image_number in range(0, NUMBER_OF_IMAGES_TO_TAKE):
            print(
                "Capturing image {image_number} for camera {camera_letter}".format(
                    image_number=image_number, camera_letter=camera_letter
                )
            )
...
for i in range(0, 5):
    ...
    if i == 4:
        break  # No action in the last step. i should be (range-1)
    print("Waiting")
    ...
    with Connection.open_serial_port("/dev/ttyUSB0") as connection:
        ...
        axis.settings.set(
            "maxspeed", 20, Units.VELOCITY_MILLIMETRES_PER_SECOND
        )  # set to gentle maximum speed, these should not have to be reset from above, but jic
        axis.settings.set(
            "motion.accelonly", 2, Units.NATIVE
        )  # set to gentle acceleration
        axis.settings.set(
            "motion.decelonly", 2, Units.NATIVE
        )  # set to gentle deceleration
        axis.move_relative(
            128, Units.LENGTH_MILLIMETRES
        )  # change back to 128 after set
...
with Connection.open_serial_port("/dev/ttyUSB0") as connection:
    ...
    axis.move_relative(
        128, Units.LENGTH_MILLIMETRES
    )  # space between the 5th row the first stage and the first row of the second stage; 210 in TO code
...
for i in range(0, 5):
    ...
    with Connection.open_serial_port("/dev/ttyUSB0") as connection:
        ...
        axis.move_relative(
            128, Units.LENGTH_MILLIMETRES
        )  # this was set to 13.5 in Toronto code bgut we don't have acetal rods between plates
    print("Waiting to settle")
    time.sleep(1)  # waiting for 1 seconds
